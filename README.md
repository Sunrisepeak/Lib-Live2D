...
mcpp 2026.9.15.2 or newer (`xlings install mcpp -y --use`). The Cubism SDKs, GLEW, esbuild, the NDK, emsdk and the JDK are payloads mcpp installs on first use; macOS and iOS additionally need Xcode with the Metal toolchain. The floors are HuxerUI's: macOS 12.0, iOS 15.0, Android API 23.
...
mcpp pack --format appimage                       # Linux; --format msi on Windows, --format dmg or app on macOS
...
One thing stays with the CMake build for now: the preview's own string catalogue (`resources/strings`), which mcpp does not merge into the application's package. The macOS and iOS bundles carry the resources and the Metal shader libraries where the CMake bundles put them (`Contents/Resources/`, the iOS root). [`mcpp/README.md`](mcpp/README.md) says how the package is put together.
